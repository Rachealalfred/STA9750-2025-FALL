...
This report analyzes Netflix's Top 10 data to highlight the streaming giant's most successful programming across the globe. Our analysis reveals compelling success stories, from the cultural phenomenon of Squid Game to Netflix's remarkable growth in the Indian market. Through detailed examination of viewership patterns across 93 countries and hundreds of titles, we identify key opportunities and celebrate milestone achievements in Netflix's content strategy.

# Introduction

As Netflix continues its mission to produce "gourmet cheeseburgers" - high-quality content with mass appeal - understanding viewership patterns becomes crucial. This analysis examines Netflix's publicly available Top 10 data to quantify recent successes and identify growth opportunities, particularly in preparation for major upcoming releases and expansion into new markets.
...
# Convert "N/A" to proper NA values
...
{r import-country}
# Read country data with NA handling
...
{r helper-functions}
# Function to format column names to Title Case
format_titles <- function(df){
  colnames(df) <- str_replace_all(colnames(df), "_", " ") |> str_to_title()
  df
}

# Create a sample table showing recent films
GLOBAL_TOP_10 |>
...
Netflix operates in *r num_countries* different countries based on the Top 10 viewing history data.
...
The non-English-language film *r film_name* has spent the most cumulative weeks in the global top 10, with a total of *r weeks_count* weeks.
...
The longest film to appear in the Netflix global Top 10 is *r longest_film_name, with a runtime of *r longest_film_minutes** minutes.
...
{r task-07}
# Find the TV show with the longest run by counting distinct weeks
...
if(!is.na(longest_country_run$season_title[1])) {
  run_show <- paste(run_show, "-", longest_country_run$season_title[1])
}


The TV show *r run_show* had the longest run in a country's Top 10, lasting *r run_weeks* weeks in *r run_country*.
...
# Find when Netflix ceased operations
last_week <- COUNTRY_TOP_10 |>
  filter(country_name == limited_country_name) |>
  summarise(last_date = max(week)) |>
  pull(last_date)


*r limited_country_name* has only *r weeks_available* weeks of data, with Netflix ceasing operations around *r format(last_week, '%B %Y')*.
...
squid_game_billion <- round(squid_game_total / 1000000000, 2)


The total viewership for all seasons of *Squid Game* is approximately *r format(squid_game_total, big.mark=",", scientific=FALSE)* hours, or *r squid_game_billion* billion hours.

### Task 10: Red Notice Views in 2021

{r task-10}
# Filter for Red Notice in 2021 - use the runtime given in assignment (1 hr 58 min = 1.97 hours)
...
if(nrow(red_notice_data) > 0) {
  ...
  runtime_hours <- 1.97  # 1 hour 58 minutes as stated in assignment
  red_notice_views <- round(total_hours / runtime_hours)
  red_notice_million <- round(red_notice_views / 1000000, 1)
} else {
  red_notice_views <- 0
  red_notice_million <- 0
}


*Red Notice* received approximately *r format(red_notice_views, big.mark=",", scientific=FALSE)* views (*r red_notice_million* million views) in 2021.

### Task 11: Films That Climbed to #1 in US

{r task-11}
# Step 1: Get all US films and their first appearance
...
# Step 2: Find which films eventually hit #1
films_that_hit_one <- COUNTRY_TOP_10 |>
  filter(country_iso2 == "US", grepl("Film", category), weekly_rank == 1) |>
  distinct(show_title)

# Step 3: Join to find climbers (debuted > 1 but eventually hit #1)
climbers <- us_films_first |>
  inner_join(films_that_hit_one, by = "show_title") |>
  filter(debut_rank > 1) |>
...
if(num_climbers > 0) {
  most_recent_climber <- climbers$show_title[1]
  most_recent_date <- climbers$debut_week[1]
  most_recent_debut_rank <- climbers$debut_rank[1]
} else {
  most_recent_climber <- "No films found"
  most_recent_date <- Sys.Date()
  most_recent_debut_rank <- 0
}


*r num_climbers* films reached #1 in the US after not debuting at the top spot. The most recent film to accomplish this was *r most_recent_climber, which debuted at rank *#r most_recent_debut_rank* in *r format(most_recent_date, '%B %Y')**.

### Task 12: Most International Debut

{r task-12}
# For each TV show/season, find when it first appeared globally
tv_shows <- COUNTRY_TOP_10 |>
  filter(grepl("TV", category))

# Find the first week each show appeared anywhere
show_first_weeks <- tv_shows |>
  group_by(show_title, season_title) |>
  summarise(first_global_week = min(week), .groups = "drop")

# Count countries in debut week for each show
debut_reach <- tv_shows |>
  inner_join(show_first_weeks, by = c("show_title", "season_title")) |>
  filter(week == first_global_week) |>
  group_by(show_title, season_title, first_global_week) |>
...
*r debut_show* hit the Top 10 in the most countries during its debut week, charting in *r debut_country_count* countries.
...
## 🎬 STRANGER THINGS PHENOMENON CONTINUES: Final Season Set to Break Records
...
::: {.callout-note icon=false}
## 🇮🇳 NETFLIX DOMINATES INDIAN MARKET: Hindi Content Drives Explosive Growth
...
::: {.callout-note icon=false}
## 🌍 NETFLIX BREAKS LANGUAGE BARRIERS: International Content Achieves Global Success
...
This cross-cultural success story demonstrates Netflix's unique ability to identify and promote content that transcends language barriers. Shows from Korea, Spain, France, and beyond now regularly appear in Top 10 lists from São Paulo to Stockholm, from Mumbai to Mexico City. Non-English content viewership has grown by *r non_english_growth$growth%* since tracking began, far outpacing the growth of English-language programming and signaling a fundamental shift in global viewing habits.
